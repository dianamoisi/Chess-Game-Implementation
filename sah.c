...
int it_is_ok(config t) {
    ...
    //verificam daca mutarile se afla in tabla de sah
    if(!(0 <= c1 && c1 < 8 && 0 <= r1 && r1 <= 8 && 0 <= c2 && c2 < 8 && 0 <= r2 && r2 < 8))
        return 0;
...
void run_tests(){
    ...
    fclose(fin);
    FILE *fout = NULL;
    fout = fopen("chess_answers_the_good_one.txt", "w");
